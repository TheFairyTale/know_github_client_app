...
class _HomeRouteState extends State<HomeRoute> {
  ...
  Widget _buildBody() {
    ...
    if (!userModel.isLogin) {
      // 用户未登陆时展示登陆按钮
      return Center(child: ElevatedButton(onPressed: () => Navigator.of(context).pushNamed("login"), child: Text(GmLocalization.of(context).login)),)
    } else {
      // 用户已登陆则显示项目列表
      ...
      }, separatorBuilder: (context, index) => Divider(height: .0,), itemCount: _items.length);
    }
  }
